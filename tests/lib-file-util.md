...
`find` returns a list of files under a directory filtered by a list of
...
Show the files.
    ...
    >>> for path in sorted(ls(".", includedirs=True)):
    ...     print(path)  # +diff
    A
    A/B
    A/B/c.txt
...
Use `find` to filter the files.

    >>> find0 = find
    >>> def find(include, exclude):
    ...     any = False
    ...     for path in sorted(find0(".", include, exclude)):
    ...         print(path)
    ...         any = True
    ...     if not any:
    ...         print("<empty>")

Empty includes list always yields nothing.
...
Files are included when they match by name.
...
Wildcards can be used to match file names.
...
Double asterisk ('**') can be used to match subdirectories.
...
Dots are not required to match files with extensions.
