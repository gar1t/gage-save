...
- Run `init-sourcecode` exec if specified
...
directory.
    ...
    ...     "sourcecode": ["train.py"],
    ...     "exec": {
    ...         "init-sourcecode": ["python", "setup.py"]
    ...     }
    ... })
...
Use `init_sourcecode()` to copy the source code and run the source code
exec.
...
`init-sourcecode` phase events are written to the runner log.
    ...
    Running init-sourcecode (see output/10_sourcecode for output):
    ['python', 'setup.py']
    Exit code for init-sourcecode: 0
...
- Run `init-deps` exec if specified

`requires` support is pending. We show how `init-deps` is used to copy
...
The op def specifies `train.py` as source code and uses `init-deps` to
run `setup.py` to generate `data.json`.

    >>> opdef = OpDef("test", {
    ...     "sourcecode": "train.py",
    ...     "exec": {
    ...         "init-deps": ["python", "setup.py", "456"]
    ...     }
    ... })

    >>> init_run_meta(run, OpRef("test", "test"), opdef, {}, OpCmd([], {}))

Use `init_deps()` to run the copy deps exec command.
...
The runner log shows copy deps info.
    ...
    Writing meta id
    ...
    Running init-deps (see output/30_deps for output): ['python', 'setup.py', '456']
    Exit code for init-deps: 0
...
Note that source code is not copied - we only called `init_deps()` up to
this point. To copy source code, use `init_sourcecode()`.
