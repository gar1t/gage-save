...
def _list_dir(
    ...
    provide listing. This is for the sake of speed as well as making the
    behavior as close as possible to the user's native shell. However,
    supporting new shells means implementing behavior for our !!
    directives. We provide native Python implementation as a fallback
    where we have not yet implemented handling for our directives.

    `filters` is None or a list of functions used to filter (select for
    inclusion) paths by returning True. Each function must accept a
    single argument, which is a pathlib.Path object for the path being
    filtered. All filters must return True for a path to be included in
    the result.
    ...
    its suffix matches any of the specified extensions. Matching in this
