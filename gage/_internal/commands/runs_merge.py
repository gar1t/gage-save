...
def merge_params(fn: Callable[..., Any]):
...
def merge_runs(ctx: click.Context, args: Any):
    """Copy run files to a project directory.

    By default, Guild copies run files into the current directory. To copy files
    to a different directory, use ``--target-dir DIR``.

    Guild checks that the run originated from the current directory before
    copying files. If the run is associated with a project from a different
    directory, or is from a package, Guild exits with an error message. In this
    case, use `--target-dir` to override the check with an explicit path.

    The command fails if any file would be replaced, unless a) the `--replace`
    option is specified or b) the replaced file is committed to the project VCS
    and unchanged. To prevent replacement even when a file is committed to VCS
    and unchanged, specify `--no-replace`.
    """
    print("TODO merge")
