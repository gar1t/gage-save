...
class JSONHelpFormatter(click.HelpFormatter):
    ...
    def write_dl(self, rows: _DLItems, col_max: int = 30, col_spacing: int = 2):
...
def format_error_message(e: click.exceptions.ClickException):
    ...
    if ctx:
        msg_parts.append(f"\nTry '{cmd_help(ctx)}' for more information.")
...
def _format_missing_parameter_error(e: click.exceptions.MissingParameter):
    name = e.param.human_readable_name if e.param else "UNKNOWN"
    return f"missing argument for {name}"


def _format_no_such_option_error(e: click.exceptions.NoSuchOption):
    if e.possibilities:
